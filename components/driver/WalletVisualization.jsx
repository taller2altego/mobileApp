...
export default function WalletVisualization({ navigation }) {
    ...
    (async () => {
      ...
      await get(`${API_URL}/users/${id}`, token).then(async (user) => {
        if (user.data.driverId) {
          setIsDriver(true);
          await get(`${API_URL}/drivers/${user.data.driverId}`, token).then(
            (data) => {
  ...
  return (
    ...
    >
      ...
      <Text
        style={[
          Profilestyles.profile_visualization,
          { fontFamily: "poppins-bold", fontSize: 30, marginTop: "10%" },
        ]}
      >
        Wallet
      </Text>
      ...
      >
        ...
        <View style={{ flexDirection: "row" }}>
          <Text
            style={{ fontFamily: "poppins", marginRight: 5, marginBottom: 10 }}
          >
            {fiuCreditos}
          </Text>
